...
SELECT emp_name, dept_id, repeat('▒', round(salary / 250))
	FROM employee
    WHERE salary > 0;
...
-- 근무중인 직원의 급여 개수 조회(같은 급여는 하나로 취급)
...
SELECT SUM(duration) AS tot_duration
	FROM vacation
    WHERE emp_id = 'S0001'
		AND begin_date LIKE '2014%';
		-- AND begin_date BETWEEN '2014-01-01' AND '2014-12-31';


-- Q) 가장 최근에 직원이 입사한 날짜 조회
SELECT MAX(hire_date) AS '가장최근입사일'
	FROM employee;
