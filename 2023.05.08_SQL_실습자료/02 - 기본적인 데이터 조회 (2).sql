...
-- IFNULL 함수 사용: eng_name 열 값이 NULL이면 공백 표시 = 두 값 중에서 첫 번째 NULL이 아닌 값
...
-- MySQL: IFNULL() 참고 : ISNULL() --> 전달된 값이 NULL이면 1, 아니면 0이 반환
-- MSSQL: ISNULL()
-- ORACLE: NVL()
*/
...
SELECT CONCAT(emp_id, '(', dept_id, ')') AS emp_id, emp_name, salary
	FROM employee
    WHERE retire_date is NULL;
...
-- eng_name이 NULL이면 공백 표시
SELECT emp_name,
	IF(eng_name IS NULL, '', eng_name) AS eng_name,
    emp_id, gender, hire_date, salary
    FROM employee;

-- eng_name이 NULL이면 공백 표시
SELECT emp_name,
	CASE WHEN eng_name IS NULL THEN '' ELSE eng_name END AS eng_name,
    emp_id, gender, hire_date, salary
    FROM employee;
...
-- Q) 급여 크기를 상, 중, 하로 구분
SELECT emp_name, emp_id, gender, hire_date, salary,
       IF(salary >= 8000,  '상', IF(salary >= 7000, '중', '하')) AS salary_level
	FROM employee;