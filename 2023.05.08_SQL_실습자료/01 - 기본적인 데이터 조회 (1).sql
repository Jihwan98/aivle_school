...
SELECT gender, emp_id, emp_name, hire_date
	FROM employee
    WHERE gender = 'M';


-- Q) 2016년 6월 1일 입사자의 이름, 부서코드, 성별, 이메일 정보 조회
SELECT hire_date, emp_name, dept_id, gender, email
	FROM employee
...
-- 다음과 같이 작성해도 되지만 권고하지는 않음(성능 문제가 있을 수 있음)
SELECT emp_name, emp_id, dept_id, hire_date, phone
	FROM employee
...
SELECT emp_name, emp_id, dept_id, hire_date, phone
	FROM employee
    WHERE hire_date >= '2018-01-01' AND dept_id = 'SYS';
...
-- 2016년에 입사한 직원을 제외한 직원 정보 조회
SELECT emp_name, emp_id, dept_id, hire_date, phone
	FROM employee
	WHERE hire_date NOT BETWEEN '2016-01-01' AND '2016-12-31';
...
-- SYS, MKT, GEN 부서 직원을 제외한 직원 정보 조회
SELECT emp_name, emp_id, dept_id, hire_date, phone
	FROM employee
...
SELECT emp_name, emp_id, dept_id, hire_date, phone, salary, retire_date
	FROM employee
...
SELECT *
    ...
    WHERE emp_id IN ('S0001', 'S0003', 'S0005')
		AND begin_date BETWEEN '2015-01-01' AND '2015-12-31'
